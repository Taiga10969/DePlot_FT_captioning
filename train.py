...
from configs import DePlot_FT_Config
from datasets import SciCapPlusDataset
from utils import torch_fix_seed, train_one_epoch, test_one_epoch

config = DePlot_FT_Config()

if config.wandb == True:
    import wandb
    wandb.init(project="train_gpt2v_version2_latest",
...
for epoch in range(1, config.num_eposh + 1, 1):
    ...
    if epoch % config.save_param_time == 0:
        model_save_path = f"./result/{config.project_name}model_epoch_{epoch}.pth"
        if config.is_DataParallel == True:
